...
// ... (images and icons object keep unchanged)
const images = [
...
export function ExperienceSection() {
  return (
    <section id="about" className="relative bg-lanka-black py-12 md:py-20 xl:py-20 2xl:py-24 3xl:py-32">
      <ContainerLayout>
        <div className="grid grid-cols-1 items-center gap-12 xl:grid-cols-2 xl:gap-16 2xl:gap-24 3xl:gap-32">
          ...
          <div className="relative order-2 xl:order-1">
            ...
            >
              {images.map((image) => (
                <div key={image.src} className={`group relative aspect-3/4 overflow-hidden rounded-2xl border border-foreground/10 bg-surface shadow-2xl md:rounded-3xl 3xl:rounded-[2.5rem] ${image.className}`}>
                  <div className="absolute inset-0 z-10 bg-linear-to-t from-lanka-dark via-transparent to-transparent" />
                  <Image src={image.src} alt={image.alt} fill sizes="(min-width: 1280px) 25vw, 50vw" className="object-cover transition-transform duration-700 group-hover:scale-110" />
                </div>
              ))}
            </motion.div>

            {/* Floating Cards (Animated Scale Up) */}
            <motion.div 
            ...
            >
              <div className="flex items-center space-x-4 3xl:space-x-6">
                ...
                <div>
                  ...
                  <p className="mt-1 text-[8px] uppercase tracking-widest text-gold 3xl:mt-2 3xl:text-[10px]">Completed Tours</p>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center space-x-4 3xl:space-x-6">
                ...
                <p className="text-[9px] font-bold uppercase leading-tight tracking-[0.2em] text-foreground/80 3xl:text-xs">Years of Luxury<br />Hosting</p>
              </div>
            </motion.div>
          </div>
          ...
          >
            <div className="mb-6 inline-block rounded-full border border-gold/20 bg-gold/5 px-4 py-1 3xl:mb-8 3xl:px-6 3xl:py-2">
              <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-gold 3xl:text-xs">Elite Travel Partner</span>
            </div>
            <h2 className="mb-6 font-serif text-3xl font-light leading-tight text-foreground sm:text-4xl md:text-5xl xl:text-5xl 2xl:text-6xl 3xl:mb-10 3xl:text-[5rem]">
              ...
              <span className="mt-1 block pb-2 pr-2 gold-gradient-text font-normal italic md:mt-2 md:pb-4 3xl:pb-6">Beyond Borders</span>
            </h2>
                ...
                return (
                  <motion.div
                    ...
                    className={`group rounded-2xl p-5 transition-all 3xl:rounded-3xl 3xl:p-8 ${
                      isHighlighted ? "border border-gold/40 bg-gold/5 shadow-[0_0_20px_rgba(197,160,89,0.1)] hover:bg-gold/10" : "border border-foreground/5 bg-foreground/2 hover:bg-foreground/5"
                    }`}
                  >
                    <div className="mb-3 flex items-center space-x-4 3xl:mb-6 3xl:space-x-6">
                      <div className={`flex h-10 w-10 items-center justify-center rounded-lg bg-surface 3xl:h-14 3xl:w-14 3xl:rounded-2xl ${isHighlighted ? "border border-gold/30" : "border border-foreground/5"}`}>
                        <Icon className={`text-gold ${pillar.title === "Private Fleet" ? "h-3.5 w-3.5 3xl:h-5 3xl:w-5" : "h-4.5 w-4.5 3xl:h-6 3xl:w-6"}`} />
                      </div>
                      <h4 className="text-xs font-bold uppercase tracking-wider text-foreground 3xl:text-base">{pillar.title}</h4>
                    </div>
                    <p className={`${isHighlighted ? "text-foreground/90" : "text-foreground/60"} text-sm font-light leading-relaxed 3xl:text-xl`}>{pillar.description}</p>
                  </motion.div>
                );
              })}
            </div>
            ...
            >
              <Button variant="inquire" href="/contact">Inquire Now <ArrowRight className="ml-3 inline h-3 w-3 transition-transform group-hover:translate-x-2 3xl:h-5 3xl:w-5" /></Button>
              <div className="text-center xl:text-left">
                <p className="text-sm font-bold text-foreground 3xl:text-xl">Response within 2 hours</p>
                <p className="text-[10px] uppercase tracking-widest text-gold/60 3xl:mt-2 3xl:text-sm">Available 24/7 Global Support</p>
              </div>
            </motion.div>
          </motion.div>
        </div>
