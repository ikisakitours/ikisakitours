...
export function PrivateVehicle() {
  ...
  useEffect(() => {
    ...
    const handleEscape = (event: KeyboardEvent) => { if (event.key === "Escape") setIsContactOpen(false); };
  ...
  return (
    <>
      <section id="private-Vehicle-Hire" className="relative overflow-hidden bg-lanka-dark py-12 md:py-20 xl:py-20 2xl:py-24 3xl:py-32">
          ...
          <div className="flex flex-col-reverse items-center gap-12 md:gap-16 xl:flex-row xl:gap-24">
            ...
            >
              <div className="relative z-10 overflow-hidden rounded-3xl border border-white/10 shadow-2xl md:rounded-[2.5rem]">
                <Image src="https://images.unsplash.com/photo-1544620347-c4fd4a3d5957?q=80&w=2000&auto=format&fit=crop" alt="Luxury Van" width={2000} height={2500} className="aspect-4/5 w-full object-cover transition-transform duration-700 hover:scale-105 sm:aspect-3/4" />
                  ...
                  <div className="mb-2 flex items-center gap-3 md:mb-3 md:gap-4">
                    <div className="flex -space-x-2">
                      {languageBadges.map((badge, index) => (
                        <span key={badge} className={`flex h-7 w-7 items-center justify-center rounded-full border-2 border-[#050505] text-[8px] font-bold text-white md:h-8 md:w-8 md:text-[10px] ${index % 2 === 0 ? "bg-slate-800" : "bg-slate-700"}`}>
                          {badge}
                        </span>
                      ))}
                    </div>
                    <span className="text-[8px] font-black uppercase tracking-[0.2em] text-gold md:text-[9px]">Native Friendly</span>
                  </div>
            ...
            >
              <header className="mb-8 md:mb-10 text-left">
                <div className="mb-4 inline-block rounded-full border border-gold/20 bg-gold/5 px-4 py-1 md:mb-6">
                  <span className="text-[9px] font-bold uppercase tracking-[0.3em] text-gold md:text-[10px]">Private Vehicle Hire</span>
                </div>
                <h2 className="mb-6 text-3xl font-light leading-[1.2] tracking-tight text-white sm:text-4xl md:mb-8 md:text-5xl xl:text-6xl xl:leading-[1.1]">
                  Your Private Vehicle <span className="italic text-gold">&amp;</span> <br className="hidden sm:block md:hidden xl:block" />
                  <span className="font-serif">Driver</span>
                </h2>
              ...
              <div className="mb-10 grid grid-cols-1 gap-6 sm:grid-cols-2 md:mb-12 md:gap-8">
                {featureCards.map(({ title, description, Icon }, index) => (
                  ...
                  >
                    ...
                    <h4 className="mb-2 text-[10px] font-bold uppercase tracking-widest text-white md:text-xs">{title}</h4>
              ...
              >
                <Button variant="inquire" href="/services/private-vehicle">Hire Your Vehicle</Button>
                <button type="button" onClick={() => setIsContactOpen(true)} className="group flex w-full cursor-pointer items-center justify-center gap-4 border-none bg-transparent p-0 text-left sm:w-auto sm:justify-start">
                  ...
                  <div className="flex flex-col">
                    <span className="whitespace-nowrap text-[10px] font-bold uppercase tracking-[0.15em] text-white/60 transition-colors group-hover:text-white md:text-[12px]">Consult With Us</span>
                    <span className="mt-1 whitespace-nowrap text-[9px] font-medium text-gold/80 md:text-[11px]">Direct WhatsApp</span>
                  </div>
                </button>
              </motion.div>
            </motion.div>
      ...
      {/* Modal - Kept Original */}
      <div id="contact-modal" className={`fixed inset-0 z-100 items-center justify-center p-4 ${isContactOpen ? "flex" : "hidden"}`} aria-hidden={!isContactOpen}>
        <button type="button" className="absolute inset-0 bg-black/90 backdrop-blur-sm" onClick={() => setIsContactOpen(false)} />
        <div className="relative w-full max-w-sm rounded-[2.5rem] border border-white/10 bg-[#111] p-8 shadow-2xl animate-[fade-in-up_0.3s_ease_both]">
           ...
           <button type="button" onClick={() => setIsContactOpen(false)} className="absolute right-6 top-6 text-slate-500 hover:text-white"><X className="h-5 w-5" /></button>
           <div className="mb-8 text-center">
             <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full border border-gold/20 bg-gold/10"><MessagesSquare className="h-5 w-5 text-gold" /></div>
             <h3 className="text-lg font-light tracking-tight text-white">How would you like to <span className="italic text-gold">connect?</span></h3>
           </div>
           <div className="space-y-3">
             <a href="https://wa.me/94789187072" className="group flex items-center justify-between rounded-2xl border border-white/5 bg-white/5 p-4 hover:border-emerald-500/50 hover:bg-emerald-500/5"><div className="flex items-center gap-4"><SiWhatsapp className="h-5 w-5 text-emerald-500" /><span className="text-xs font-bold uppercase tracking-widest text-white">WhatsApp</span></div></a>
             <a href="mailto:info@elitetransfers.lk" className="group flex items-center justify-between rounded-2xl border border-white/5 bg-white/5 p-4 hover:border-[#c5a059]/50 hover:bg-[#c5a059]/5"><div className="flex items-center gap-4"><MessagesSquare className="h-4.5 w-4.5 text-[#c5a059]" /><span className="text-xs font-bold uppercase tracking-widest text-white">Email</span></div></a>
           </div>
           <p className="mt-8 text-center text-[10px] leading-relaxed text-slate-500">Our team typically responds <br /> within 15 minutes.</p>
        </div>
      </div>
    </>
  );
