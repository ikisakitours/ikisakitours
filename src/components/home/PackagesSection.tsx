...
export function PackagesSection() {
  const displayPackages = packages.filter((p) => Number(p.rating) === 5).slice(0, 4);
  return (
    <section id="packages" className="bg-background py-12 md:py-20 xl:py-20 2xl:py-24 3xl:py-32">
      <ContainerLayout>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4 3xl:gap-10">
          {displayPackages.slice(0, 4).map((item, index) => (
            <motion.article
              key={item.title}
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
        ...
        >
          <Button variant="explore" href="/packages">
            Visit Our Full Package Page
          </Button>
        </motion.div>
      </ContainerLayout>
    </section>
