...
export function ChatHome({ onNewChat }: { onNewChat: () => void }) {
  const t = useTranslations("ChatWidget.Home");
  return (
    <div className="animate-fade-in-up flex flex-col justify-between min-h-[360PX] ">
      <div className="bg-linear-to-br from-gold-dark/40 via-surface to-lanka-black border border-gold/20 rounded-2xl p-5 relative overflow-hidden shadow-lg flex flex-col justify-between h-full">
        ...
        <div>
          <ChatImageSlider />
          <h2 className="text-2xl font-bold text-white mb-2 premium-serif">{t("greeting")}</h2>
         ...
         {t("description")}
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gold/5 border border-gold/20 text-[10px] font-bold uppercase tracking-wider text-gold">
              <Zap size={12} />
              <span>{t("badges.quickReplies")}</span>
            </div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gold/5 border border-gold/20 text-[10px] font-bold uppercase tracking-wider text-gold">
              <ShieldCheck size={12} />
              <span>{t("badges.safeSecure")}</span>
            </div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gold/5 border border-gold/20 text-[10px] font-bold uppercase tracking-wider text-gold">
              <Headset size={12} />
              <span>{t("badges.service")}</span>
            </div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gold/5 border border-gold/20 text-[10px] font-bold uppercase tracking-wider text-gold">
              <Compass size={12} />
              <span>{t("badges.tailoredTours")}</span>
            </div>
          </div>
        </div>
        ...
        >
          <div className="text-left">
            <p className="font-bold text-lanka-black text-[13px]">{t("newConversation")}</p>
            <p className="text-[11px] text-slate-500 mt-0.5">{t("replyTime")}</p>
          </div>
