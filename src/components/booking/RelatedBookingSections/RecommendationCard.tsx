...
export default function RecommendationCard({ item }: { item: RecommendationType }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-6">
        <div className="mb-2 text-tiny font-bold uppercase tracking-[0.3em] text-gold">{item.origin}</div>
        <h3 className="premium-serif mb-3 text-heading-card font-bold leading-tight text-white">
          {item.title} <br />
          <span className=" text-heading-card font-light italic">{item.subtitle}</span>
        </h3>
        ...
        <div className="mb-6 flex items-center justify-between gap-6 text-caption font-bold uppercase tracking-widest text-slate-400">
          ...
          <span className="group relative flex shrink-0 items-center justify-center overflow-hidden rounded border border-white/10 bg-white/5 backdrop-blur-sm transition-all duration-500 hover:bg-gold hover:border-gold min-h-11 px-5 py-3 text-tiny! sm:min-h-0 sm:px-5 sm:py-2.5 font-bold uppercase tracking-[0.2em] text-white">
            ...
            <ArrowRight
              strokeWidth={2}
              className="ml-2 h-3.5 w-3.5 relative z-10 transition-colors duration-300 group-hover:text-white"
            />
          </span>
        </div>
        <div className="flex items-center justify-between border-t border-white/10 pt-5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-body font-bold tracking-widest text-white">{item.rating}</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-heading-card font-bold tracking-tighter text-gold leading-none">{item.price}</span>
          </div>
        </div>
      </div>
    </Link>
