...
import { useLocale } from "next-intl"; // මෙය අලුතින් Import කළ යුතුමයි
import { languages } from "@/data/Languages-CurrencyData";
//Icons
import { ChevronDown, Search } from "lucide-react";
...
export function LanguageSelector() {
  ...
  const [isOpen, setIsOpen] = useState(false);
  // ආරම්භක භාෂාව URL එකේ තිබෙන භාෂාවට (currentLocale) සමාන කර ඇත
  const [baseLang, setBaseLang] = useState(currentLocale.toUpperCase());
  const [search, setSearch] = useState("");
  ...
  // වර්තමාන භාෂාව සොයාගැනීම (Case insensitive)
  const currentLang =
    languages.find((l) => l.code.toLowerCase() === baseLang.toLowerCase()) || languages[0];
    ...
    <div className="relative flex items-center z-70" ref={dropdownRef}>
      ...
      <button
        ...
        className={`group relative flex items-center gap-1.5 rounded-full border pl-3.5 pr-2.5 py-1.75 transition-all duration-300 focus:outline-none ${
          isOpen
            ...
            : "bg-white/5 border-white/10 hover:border-gold/30 hover:bg-gold/5"
      ...
      >
        ...
        <div className="max-h-64 overflow-y-auto py-1 custom-scrollbar">
          {filteredLanguages.length > 0 ? (
            filteredLanguages.map((lang) => {
              const isSelected = baseLang.toLowerCase() === lang.code.toLowerCase();
              return (
                <button
                  key={lang.code}
                  ref={isSelected ? selectedItemRef : null}
                  onClick={() => {
                    const newLocale = lang.code.toLowerCase(); // 'EN' -> 'en'
                    
                    // UI State සහ අනෙකුත් Functions එලෙසම යාවත්කාලීන කිරීම
                    setBaseLang(lang.code);
                    setIsOpen(false);
                    setSearch("");
                    window.dispatchEvent(
                      new CustomEvent("languageChanged", { detail: lang.currency })
                    );

                    // next-intl හරහා URL එක සහ භාෂාව මාරු කිරීම
