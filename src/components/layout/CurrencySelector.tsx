...
import { allCurrencies } from "@/data/Languages-CurrencyData";
//Icons
...
export function CurrencySelector() {
  ...
  useEffect(() => {
    const handleLanguageChange = (e: Event) => {
      ...
      if (customEvent.detail) {
        setBaseCurrency(customEvent.detail);
      }
    };
  ...
  const filteredCurrencies = useMemo(() => {
    ...
    return allCurrencies.filter((c) => c.name.toLowerCase().includes(q) || c.code.toLowerCase().includes(q));
  }, [search]);

  const currentSymbol = allCurrencies.find((c) => c.code === baseCurrency)?.symbol || "$";
  ...
  return (
    <div className="relative flex items-center z-70" ref={dropdownRef}>
      ...
      >
        ...
        <span className="text-[13px] font-bold text-white transition-colors group-hover:text-gold">{baseCurrency}</span>
        <span className="text-[12px] font-bold text-gold/90">({currentSymbol})</span>
        <ChevronDown
      ...
      >
        <div className="border-b border-white/10 bg-white/5 p-3">
          <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-slate-400">Select Currency</p>
          <div className="relative flex items-center">
            <Search className="absolute left-3 h-3.5 w-3.5 text-slate-400" />
            <input
              type="text"
              placeholder="Search currency (e.g. LKR, Euro)..."
              value={search}
        ...
        <div className="max-h-64 overflow-y-auto py-1 custom-scrollbar">
          {filteredCurrencies.length > 0 ? (
              ...
              return (
                ...
                >
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-slate-500 font-semibold">{currency.code}</span>
                  </div>
            ...
            <div className="px-4 py-4 text-center text-sm text-slate-500">No currency found</div>
          )}
        </div>
      </div>
    </div>
